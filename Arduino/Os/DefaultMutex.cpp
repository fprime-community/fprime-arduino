...
MutexInterface *MutexInterface::getDelegate(MutexHandleStorage& aligned_new_memory) {
    return Os::Delegate::makeDelegate<MutexInterface, Os::Arduino::Mutex::ArduinoMutex>(
            aligned_new_memory
    );
}
...
ConditionVariableInterface *ConditionVariableInterface::getDelegate(ConditionVariableHandleStorage& aligned_new_memory) {
    return Os::Delegate::makeDelegate<ConditionVariableInterface, Os::Arduino::Mutex::ArduinoConditionVariable, ConditionVariableHandleStorage>(
        aligned_new_memory
    );
}
}
